 ...
 * 双数组Trie树
 */
public class DoubleArrayTrie<V> implements Serializable, Trie<V> {
...
//                System.out.println(this);
    ...
    public void open(Path file) throws IOException {
        size = (int) FileHelper.fileSize(file) / UNIT_SIZE;
        check = new int[size];
        base = new int[size];

        try (DataInputStream is = new DataInputStream(new BufferedInputStream(Files.newInputStream(file), BUF_SIZE))) {
     ...
     * @return
     */
    public boolean load(Path file, List<V> value) {
        if (!loadBaseAndCheck(file)) return false;
        v = (V[]) value.toArray();
        return true;
    }
     ...
     * @return
     */
    public boolean load(Path path, V[] value) {
    ...
    public boolean load(byte[] bytes, int offset, V[] value) {
        ...
        size = ByteHelper.bytesHighFirstToInt(bytes, offset);
        offset += 4;
        base = new int[size + 65535];   // 多留一些，防止越界
        check = new int[size + 65535];
        for (int i = 0; i < size; i++) {
            base[i] = ByteHelper.bytesHighFirstToInt(bytes, offset);
            offset += 4;
            check[i] = ByteHelper.bytesHighFirstToInt(bytes, offset);
            offset += 4;
        }
        v = value;
        return true;
    ...
    public boolean load(Path file) {
        return loadBaseAndCheckByFileChannel(file);
    }
     ...
     * @return
     */
    private boolean loadBaseAndCheck(Path file) {
    ...
    private boolean loadBaseAndCheckByFileChannel(Path file) {
        try (FileInputStream fis = new FileInputStream(file.toFile())) {
            ...
            int index = 0;
            size = ByteHelper.bytesHighFirstToInt(bytes, index);
            index += 4;
            base = new int[size + 65535];   // 多留一些，防止越界
            check = new int[size + 65535];
            for (int i = 0; i < size; i++) {
                base[i] = ByteHelper.bytesHighFirstToInt(bytes, index);
                index += 4;
                check[i] = ByteHelper.bytesHighFirstToInt(bytes, index);
                index += 4;
            }
        } catch (Exception e) {
            return false;
     ...
     * @return
     */
    public boolean serializeTo(Path file) {
        return FileHelper.writeObject(this, file);
    }

    public static <T> DoubleArrayTrie<T> unSerialize(Path file) {
        return FileHelper.readObject(file);
    }
